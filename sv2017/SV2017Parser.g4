...
// NOTICE : Added limited support for pre-processor parsing
...
description:
   ...
   | program_declaration
   | package_declaration
   | ( attribute_instance )* ( package_item_item | bind_directive )
   | config_declaration
;
...
timescale_compiler_directive:   
  TICK_TIMESCALE time_lit DIV time_lit ;

default_nettype_statement:   
  TICK_DEFAULT_NETTYPE net_type ;

include_svh:   
  TICK_INCLUDE STRING_LITERAL ;
...
timeunits_declaration:
    KW_TIMEUNIT time_lit ( ( DIV
                                 | SEMI KW_TIMEPRECISION
                                 ) time_lit )? SEMI
    | KW_TIMEPRECISION time_lit SEMI ( KW_TIMEUNIT time_lit SEMI )?
;
lifetime:
    KW_STATIC
    | KW_AUTOMATIC
...
udp_declaration:
 KW_EXTERN ( udp_nonansi_declaration
  ...
  | ( ( udp_nonansi_declaration udp_port_declaration
      ...
      | udp_ansi_declaration
      ) udp_body KW_ENDPRIMITIVE ( COLON identifier | /* { input.LA(1) != COLON }? */ )
;
...
statement_item:
  ...
  | macro_statement                      #StmtItemMacro
  | case_statement                       #StmtItemCase
  | conditional_statement                #StmtItemCond
  | subroutine_call_statement            #StmtItemSubCall
  | disable_statement                    #StmtItemDisable
  | event_trigger                        #StmtItemEvent
  | loop_statement                       #StmtItemLoop
  | jump_statement                       #StmtItemJump
  | par_block                            #StmtItemPar
  | procedural_timing_control_statement  #StmtItemProcTime
  | seq_block                            #StmtItemSeq
  | wait_statement                       #StmtItemWait
  | procedural_assertion_statement       #StmtItemProcAssert
  | randsequence_statement               #StmtItemRandseq
  | randcase_statement                   #StmtItemRandcase
  | expect_property_statement            #StmtItemExpect
;

macro_statement:
  TICK_IDENTIFIER ( LPAREN data_type (COMMA list_of_arguments)?
...
clockvar_expression: hierarchical_identifier select_;
...
action_block:
   ...
   | statement_or_null ( KW_ELSE statement_or_null | /* { input.LA(1) != KW_ELSE }? */ )
;
seq_block:
    KW_BEGIN ( COLON identifier | /* { input.LA(1) != COLON }? */ )
        ( block_item_declaration )* ( statement_or_null )* 
    KW_END (COLON identifier |  /* { input.LA(1) != COLON }? */ );
par_block:
    KW_FORK ( COLON identifier | /* { input.LA(1) != COLON }? */ )
        ( block_item_declaration )* ( statement_or_null )* 
    join_keyword ( COLON identifier |  /* { input.LA(1) != COLON }? */ );
/******************************************** case ********************************************************************/
case_statement:
    ( unique_priority )?
...
conditional_statement:
    ...
    ( KW_ELSE statement_or_null | /* { input.LA(1) != KW_ELSE }? */ );
...
rs_if_else:
    ...
    ( KW_ELSE production_item | /* { input.LA(1) != KW_ELSE }? */ );
rs_repeat:
...
parameter_declaration_primitive:
    ( KW_TYPE list_of_type_assignments
    | ( data_type_or_implicit )? list_of_param_assignments
    );
local_parameter_declaration: KW_LOCALPARAM parameter_declaration_primitive ;
parameter_declaration: KW_PARAMETER parameter_declaration_primitive ;
type_declaration:
    KW_TYPEDEF (
...
attribute_instance: 
  ...
  | /* { input.LA(2) != LPAREN }? */ TICK_IDENTIFIER
;
...
primary:
    ...
    | primary LSQUARE_BR array_range_expression RSQUARE_BR #PrimaryRange
...
expression:
    ...
    | LPAREN operator_assignment RPAREN                                   #ExpressionAssign
    | KW_TAGGED identifier ( expression )?                                #ExpressionTaggedId
    | unary_operator ( attribute_instance )* primary                      #ExpressionUnary
    | inc_or_dec_expression                                               #ExpressionIncDec
    | expression DOUBLESTAR           ( attribute_instance )* expression  #ExpressionBinOpPower
    | expression operator_mul_div_mod ( attribute_instance )* expression  #ExpressionBinOpMul
    | expression operator_plus_minus  ( attribute_instance )* expression  #ExpressionBinOpAdd
    | expression operator_shift       ( attribute_instance )* expression  #ExpressionBinOpShift
    | expression operator_cmp         ( attribute_instance )* expression  #ExpressionBinOpCmp
    | expression KW_INSIDE LBRACE open_range_list RBRACE                  #ExpressionInside
    | expression operator_eq_neq      ( attribute_instance )* expression  #ExpressionBinOpEq
    | expression AMPERSAND            ( attribute_instance )* expression  #ExpressionBinOpBitAnd
    | expression operator_xor         ( attribute_instance )* expression  #ExpressionBinOpBitXor
    | expression BAR                  ( attribute_instance )* expression  #ExpressionBinOpBitOr
...
variable_lvalue:
 LBRACE variable_lvalue ( COMMA variable_lvalue )* RBRACE #VarLConcat
  | package_or_class_scoped_hier_id_with_select           #VarLPath
  | ( assignment_pattern_expression_type )? assignment_pattern_variable_lvalue #VarLAssign
  | streaming_concatenation                               #VarLStreamConcat
;
solve_before_list:
...
constraint_expression:
    ...
    ( KW_ELSE constraint_set | /* { input.LA(1) != KW_ELSE }? */ )
...
module_declaration:
  KW_EXTERN module_header_common ( list_of_port_declarations )? SEMI
   | module_header_common (list_of_port_declarations | (LPAREN DOT MUL RPAREN) )? SEMI
     ...
     KW_ENDMODULE ( COLON identifier | /* { input.LA(1) != COLON }? */ )
;
module_keyword:
  KW_MODULE
   | KW_MACROMODULE
...
net_declaration:
 ( KW_INTERCONNECT ( implicit_data_type )? ( HASH delay_value )? identifier ( unpacked_dimension )* (
  COMMA identifier ( unpacked_dimension )* )? /* unsupported  */
...
parameter_port_declaration:
    KW_TYPE list_of_type_assignments       #ParamPortType
    | parameter_declaration                #ParamSimple
    | local_parameter_declaration          #ParamLocal
    | data_type list_of_param_assignments  #ParamAssign
;
...
function_declaration:
    ...
    KW_ENDFUNCTION ( COLON identifier | /* { input.LA(1) != COLON }? */ )
;
...
task_declaration:
    KW_TASK ( lifetime )?
	task_and_function_declaration_common
    KW_ENDTASK ( COLON identifier | /* { input.LA(1) != COLON }? */ )
;
method_prototype:
    task_prototype
    | function_prototype
...
checker_declaration:
    ...
    KW_ENDCHECKER ( COLON identifier | /* { input.LA(1) != COLON }? */ );
class_declaration:
    ...
    ( class_item )*
    KW_ENDCLASS ( COLON identifier | /* { input.LA(1) != COLON }? */ );
always_construct:
...
package_declaration:
 ...
 ( package_item )*
  KW_ENDPACKAGE ( COLON identifier | /* { input.LA(1) != COLON }? */ )
;
package_item:
  ( attribute_instance )* package_item_item;
...
program_declaration:
 KW_EXTERN program_header
  | ( ( program_header
      ...
      ) KW_ENDPROGRAM ( COLON identifier | /* { input.LA(1) != COLON }? */ )
;
program_header:
...
genvar_iteration:
    ...
    | inc_or_dec_operator identifier                    #GenIterPrefix
;
conditional_generate_construct:
...
if_generate_construct:
    ...
    ( KW_ELSE generate_item | /* { input.LA(1) != KW_ELSE }? */ );
case_generate_construct:
...
generate_begin_end_block:
    ( identifier COLON )? KW_BEGIN ( COLON identifier | /* { input.LA(1) != COLON }? */ )
        ( generate_item )*
    KW_END ( COLON identifier | /* { input.LA(1) != COLON }? */ )
;
...
generate_item:
    ( attribute_instance )* generate_item_item #GenItemItem
    | KW_RAND data_declaration                 #GenItemDataDecl
    | generate_region                          #GenItemGenReg
    | generate_begin_end_block                 #GenItemGenBlock
;
program_generate_item:
    loop_generate_construct
    | conditional_generate_construct
...
/************************************************* module item  *******************************************************/
core_item_item:
    parameter_override                   #CoreItemParamOver
    | gate_instantiation                 #CoreItemGate
    | udp_instantiation                  #CoreItemUdp
    | module_or_interface_or_program_or_udp_instantiation #CoreItemInstance
    | ( default_clocking_or_dissable_construct
        | local_parameter_declaration
        | parameter_declaration )? SEMI  #CoreItemParam
    | net_declaration                    #CoreItemNet
    | data_declaration                   #CoreItemData
    | task_declaration                   #CoreItemTask
    | function_declaration               #CoreItemFunction
    | checker_declaration                #CoreItemChecker
    | dpi_import_export                  #CoreItemDPI
    | extern_constraint_declaration      #CoreItemExtern
    | class_declaration                  #CoreItemClass
    | interface_class_declaration        #CoreItemIntf
    | class_constructor_declaration      #CoreItemClassCons
    | covergroup_declaration             #CoreItemCover
    | property_declaration               #CoreItemProperty
    | sequence_declaration               #CoreItemSeq
    | let_declaration                    #CoreItemLet
    | genvar_declaration                 #CoreItemGenVar
    | clocking_declaration               #CoreItemClock
    | assertion_item                     #CoreItemAssert
    | bind_directive                     #CoreItemBind
    | continuous_assign                  #CoreItemContinuous
    | net_alias                          #CoreItemNetAlias
    | initial_construct                  #CoreItemInitial
    | final_construct                    #CoreItemFinal
    | always_construct                   #CoreItemAlways
    | loop_generate_construct            #CoreItemLoop
    | conditional_generate_construct     #CoreItemCondGen
    | elaboration_system_task            #CoreItemElab
;
...
module_item:
    generate_region                                 #ModuleGenReg
    | ( attribute_instance )* module_item_item      #ModuleItemItem
    | specify_block                                 #ModuleSpecBlock
    | program_declaration                           #ModuleProgDecl
    | module_declaration                            #ModuleDecl
    | interface_declaration                         #ModuleIntfDecl
    | timeunits_declaration                         #ModuleTimeDecl
    | nonansi_port_declaration SEMI                 #ModulePortDecl
;
