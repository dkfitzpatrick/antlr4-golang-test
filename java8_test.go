
package main

import (
	"fmt"
	"strconv"
	"testing"
	
	// "github.com/pkg/profile"
	"github.com/antlr/antlr4/runtime/Go/antlr"
	"github.com/dkfitzpatrick/antlr4-golang-test/java8"
)

type SourceListener struct {
	*java8.BaseJava8ParserListener
	FileName       string
}

type SourceErrorListener struct {
	antlr.ErrorListener
}

func (d *SourceErrorListener) SyntaxError(recognizer antlr.Recognizer, offendingSymbol interface{}, 
	line, column int, msg string, e antlr.RecognitionException) {
	panic("line " + strconv.Itoa(line) + ":" + strconv.Itoa(column) + " " + msg)
}


func benchmarkSource(b *testing.B, file string) {
        // defer profile.Start(profile.MemProfile, profile.ProfilePath(".")).Stop()
        // defer profile.Start(profile.CPUProfile, profile.ProfilePath(".")).Stop()
	fmt.Printf("loading:  %s\n", file)
	input, _ := antlr.NewFileStream(file)
	lexer := java8.NewJava8Lexer(input)
	stream := antlr.NewCommonTokenStream(lexer, 0)
	prsr := java8.NewJava8Parser(stream)

	prsr.BuildParseTrees = true
	tree := prsr.Source_text()
	listener := &SourceListener{
		FileName: file,
	}
	antlr.ParseTreeWalkerDefault.Walk(listener, tree)
}

func BenchmarkJava8Clock(b *testing.B) {
	benchmarkSource(b, "java8/data/LeftRecursiveRuleAnalyzer.java")
}

func BenchmarkJava8CNN(b *testing.B) {
	benchmarkSource(b, "java8/data/ParserATNFactory.java")
}

